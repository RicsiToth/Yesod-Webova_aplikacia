...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { eL: 'hidden', eu: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { eL: 'mozHidden', eu: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { eL: 'msHidden', eu: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { eL: 'webkitHidden', eu: 'webkitvisibilitychange' }
		: { eL: 'hidden', eu: 'visibilitychange' };
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		function done(response) {
			callback(toTask(request.eG.a(response)));
		}

		var xhr = new XMLHttpRequest();
		xhr.addEventListener('error', function() { done($elm$http$Http$NetworkError_); });
		xhr.addEventListener('timeout', function() { done($elm$http$Http$Timeout_); });
		xhr.addEventListener('load', function() { done(_Http_toResponse(request.eG.b, xhr)); });
		$elm$core$Maybe$isJust(request.ea) && _Http_track(router, xhr, request.ea.a);

		try {
			xhr.open(request.eU, request.ft, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {dl: fragment, dp: host, dL: path, dN: port_, dR: protocol, dS: query};
	});
			...
			return $elm$http$Http$Request(
				{
					eo: r.eo,
					er: r.er,
					eG: A2(_Http_mapExpect, func, r.eG),
					dn: r.dn,
					eU: r.eU,
					fk: r.fk,
					ea: r.ea,
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{eo: false, er: r.er, eG: r.eG, dn: r.dn, eU: r.eU, fk: r.fk, ea: r.ea, ft: r.ft}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{er: $elm$http$Http$emptyBody, eG: r.eG, dn: _List_Nil, eU: 'GET', fk: $elm$core$Maybe$Nothing, ea: $elm$core$Maybe$Nothing, ft: r.ft});
};
var $author$project$Main2$Data = F2(
	function (time, value) {
		return {d9: time, ed: value};
	});
...
var $elm$parser$Parser$Advanced$DeadEnd = F4(
	function (row, col, problem, contextStack) {
		return {c8: col, ey: contextStack, dO: problem, dZ: row};
	});
...
var $elm$parser$Parser$Advanced$fromState = F2(
	function (s, x) {
		return A2(
			...
			A4($elm$parser$Parser$Advanced$DeadEnd, s.dZ, s.c8, x, s.f));
	});
		...
		while (true) {
			...
			if (_Utils_eq(newOffset, -1)) {
				return A3(
					...
					0,
					{c8: col, f: s0.f, n: s0.n, b: offset, dZ: row, a: s0.a});
			} else {
...
var $elm$parser$Parser$Advanced$chompWhile = function (isGood) {
	return function (s) {
		return A5($elm$parser$Parser$Advanced$chompWhileHelp, isGood, s.b, s.dZ, s.c8, s);
	};
};
	...
	function (isGood, expecting) {
		return function (s) {
			...
			return _Utils_eq(newOffset, -1) ? A2(
				...
				true,
				0,
				{c8: 1, f: s.f, n: s.n, b: s.b + 1, dZ: s.dZ + 1, a: s.a}) : A3(
				$elm$parser$Parser$Advanced$Good,
				true,
				0,
				{c8: s.c8 + 1, f: s.f, n: s.n, b: newOffset, dZ: s.dZ, a: s.a}));
		};
	});
...
var $elm$parser$Parser$Advanced$token = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, str, s.b, s.dZ, s.c8, s.a);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		return _Utils_eq(newOffset, -1) ? A2(
			...
			progress,
			0,
			{c8: newCol, f: s.f, n: s.n, b: newOffset, dZ: newRow, a: s.a});
	};
};
...
var $elm$parser$Parser$DeadEnd = F3(
	function (row, col, problem) {
		return {c8: col, dO: problem, dZ: row};
	});
var $elm$parser$Parser$problemToDeadEnd = function (p) {
	return A3($elm$parser$Parser$DeadEnd, p.dZ, p.c8, p.dO);
};
...
var $elm$parser$Parser$Advanced$run = F2(
	function (_v0, src) {
		var parse = _v0;
		var _v1 = parse(
			{c8: 1, f: _List_Nil, n: 1, b: 0, dZ: 1, a: src});
		if (!_v1.$) {
			var value = _v1.b;
...
var $author$project$Main2$getData = function (devId) {
	return $elm$http$Http$get(
		{
			eG: A2($elm$http$Http$expectJson, $author$project$Main2$GotData, $author$project$Main2$theDecoder),
...
var $author$project$Main2$Again = {$: 0};
...
var $folkertdev$elm_deque$Internal$rebalance = function (deque) {
	...
	if ((sizeF + sizeR) < 2) {
		...
		if (_Utils_cmp(sizeF, (balanceConstant * sizeR) + 1) > 0) {
			...
			return {N: newFront, P: newRear, G: size1, H: size2};
		} else {
...
var $folkertdev$elm_deque$Deque$pushBack = F2(
	function (elem, _v0) {
		...
		return A2(
			...
			{
				N: deque.N,
				P: A2($elm$core$List$cons, elem, deque.P),
				G: deque.G,
				H: deque.H + 1
			});
	});
...
var $folkertdev$elm_deque$Deque$append = F2(
	function (p, q) {
		...
		return $folkertdev$elm_deque$Deque$isEmpty(p) ? q : ($folkertdev$elm_deque$Deque$isEmpty(q) ? p : {
			N: _Utils_ap(
				x.N,
				$elm$core$List$reverse(x.P)),
			P: $elm$core$List$reverse(
				_Utils_ap(
					y.N,
					$elm$core$List$reverse(y.P))),
			G: x.G + x.H,
			H: y.G + y.H
		});
	});
...
var $folkertdev$one_true_path_experiment$Curve$slope2 = F3(
	function (p0, p1, t) {
		var _v0 = $ianmackenzie$elm_geometry$Vector2d$components(
...
var $folkertdev$one_true_path_experiment$Curve$slope3 = F3(
	function (p0, p1, p2) {
		...
		var dx2 = _v0.a;
		var dy2 = _v0.b;
		var _v1 = $ianmackenzie$elm_geometry$Vector2d$components(
...
var $author$project$Main2$maximumTime = function (list) {
	if (!list.b) {
		...
		if (!list.b.b) {
			...
			var x = _v2.a;
			var xs = list.b;
			var valueMillis = $elm$time$Time$posixToMillis(x);
			var maxTail = $author$project$Main2$maximumTime(xs);
			var maxTailMillis = $elm$time$Time$posixToMillis(maxTail);
			return (_Utils_cmp(valueMillis, maxTailMillis) > 0) ? x : maxTail;
		}
	}
};
var $author$project$Main2$minimumTime = function (list) {
	if (!list.b) {
		...
		if (!list.b.b) {
			...
			return (_Utils_cmp(valueMillis, minTailMillis) < 0) ? x : minTail;
		}
	}
};
...
var $justinmimbs$date$Date$toCalendarDate = function (_v0) {
	...
	return A3($justinmimbs$date$Date$toCalendarDateHelp, date.em, 0, date.ct);
};
	...
	function (unit, n, _v0) {
		...
		switch (unit) {
			...
			case 1:
				...
				var wholeMonths = ((12 * (date.em - 1)) + ($justinmimbs$date$Date$monthToNumber(date.dE) - 1)) + n;
...
var $gampleman$elm_visualization$Scale$time = F3(
	function (zone, range_, domain_) {
		return {
			I: $gampleman$elm_visualization$Scale$Time$convert,
			b0: domain_,
			bD: $gampleman$elm_visualization$Scale$Time$invert,
			bj: $gampleman$elm_visualization$Scale$Time$nice(zone),
			y: range_,
			aY: $gampleman$elm_visualization$Scale$Time$rangeExtent,
			cV: $gampleman$elm_visualization$Scale$Time$tickFormat(zone),
			cW: $gampleman$elm_visualization$Scale$Time$ticks(zone)
		};
	});
...
var $author$project$Main2$maximumFloat = function (list) {
	if (!list.b) {
		...
		if (!list.b.b) {
			...
			return (_Utils_cmp(y, maxTail) > 0) ? y : maxTail;
		}
	}
};
var $author$project$Main2$minimumFloat = function (list) {
	if (!list.b) {
		return 0;
	} else {
		if (!list.b.b) {
			...
			var y = _v2.b;
			var xs = list.b;
			var minTail = $author$project$Main2$minimumFloat(xs);
			return (_Utils_cmp(y, minTail) < 0) ? y : minTail;
		}
	}
};
var $author$project$Main2$yScale = function (model) {
	var min = $author$project$Main2$minimumFloat(model);
...
var $folkertdev$elm_deque$Internal$empty = {N: _List_Nil, P: _List_Nil, G: 0, H: 0};
...
var $folkertdev$svg_path_lowlevel$Path$LowLevel$decimalPlaces = $elm$core$Basics$identity;
var $folkertdev$one_true_path_experiment$SubPath$defaultConfig = {bA: $elm$core$Maybe$Nothing, bH: false};
...
var $folkertdev$svg_path_lowlevel$Path$LowLevel$defaultConfig = {bf: $elm$core$String$fromFloat};
...
var $folkertdev$svg_path_lowlevel$Path$LowLevel$optionFolder = F2(
	function (option, config) {
		...
		return _Utils_update(
			config,
			{
				bf: $folkertdev$svg_path_lowlevel$Path$LowLevel$roundTo(n)
			});
	});
...
var $folkertdev$svg_path_lowlevel$Path$LowLevel$stringifyCoordinate = F2(
	function (config, _v0) {
		var x = _v0.a;
		var y = _v0.b;
		return config.bf(x) + (',' + config.bf(y));
	});
...
var $gampleman$elm_visualization$Axis$verticalAttrs = {
	...
	el: A2($elm$core$Basics$composeL, $elm$svg$Svg$Attributes$x, $elm$core$String$fromFloat),
...
var $gampleman$elm_visualization$Axis$horizontalAttrs = {
	...
	el: A2($elm$core$Basics$composeL, $elm$svg$Svg$Attributes$y, $elm$core$String$fromFloat),
...
var $author$project$Main2$main = $elm$browser$Browser$element(
	{eP: $author$project$Main2$init, fh: $author$project$Main2$subscriptions, fs: $author$project$Main2$update, fu: $author$project$Main2$view});
